...
probability.data[which(probability.data$likelihood==max(probability.data$likelihood)),]
#0.5
...
likelihood <- function(x) {
  likelihood.distribution <- dbinom(same.diff.data, 40, x)
  likelihood.data <- prod(likelihood.distribution)
  print(likelihood.data)
}
...
same.diff <- data.frame(theta)
theta = seq(from = 0.5, to = 0.9, by = 0.01)
same.diff$likelihood <- sapply(theta, likelihood)

same.diff[which(same.diff$likelihood==max(same.diff$likelihood)),]
#    theta   likelihood
#27  0.76 1.641521e-20

# the "true" underlying value i used to generate the data was 0.75. does that match up with the grid search?
# It doesn't match up exactly.. why??
# But it's close. Maybe there's a limit to the precision?
...
y.noise <- rnorm(101, mean=0, sd=10)

# plot the data again, with the new noisy y values.

plot(x = x, y = y + y.noise)
...
likelihood.calculator <- function(x, y){
  likelihood <- dnorm(y, y.predicted, 10)
  return(likelihood)
}

x.observed <- c(0:100)
y.predicted <- 4 + 0.8*x.observed
y.observed <- rnorm(101, mean=y.predicted, sd=10)

likelihood.data <- likelihood.calculator(x.observed, y.observed)
